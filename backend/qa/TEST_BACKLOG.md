...
- [ ] **B13** — `GET /api/v1/me` → 200, returns the caller's user object (assert `id` + `email`). (read-only)
- [ ] **B14** — `GET /api/v1/events/public` → 200, JSON collection. (read-only)
...
- [ ] **F11** — `rounds/[id]` (round detail — **#1 most-visited screen, 99 visits/30d**): navigate to the seeded fixture round and assert the round-detail chrome renders (round name/course header + a leaderboard or scores anchor). (auth — needs the detail-screen fixture)
...
- [ ] **F14** — `rounds/create` (22 visits/30d): open the round-creation form (via the Create affordance) and assert its fields render, then dismiss without creating. (auth — data-independent, no mutation)
