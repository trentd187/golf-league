...
- [x] **B16** — `GET /api/v1/users/:self/stats?filter=all_time` → 200, stats object. Capture `:self` from `GET /me` first. (read-only) — `backend/qa/user_stats.hurl` (added 2026-07-05; captures self id from `GET /me`, asserts the always-present integer counters `rounds_counted`/`birdies`/`pars` — the nullable averages are pointers, the counts are not, so a user with no completed rounds still returns them)
- [x] **B17** — `GET /api/v1/users/:self/rounds` → 200, JSON collection (`:self` from `GET /me`). (read-only) — `backend/qa/user_rounds.hurl` (added 2026-07-08; mirrors the B16 self-id capture, asserts `$` isCollection — a user with no rounds still returns an empty collection)
- [x] **B18** — `GET /api/v1/users/:self/scorecards` → 200, JSON collection — the batched endpoint behind the N+1 stats fix (`:self` from `GET /me`). (read-only) — `backend/qa/user_scorecards.hurl` (added 2026-07-08; mirrors B16/B17 self-id capture, asserts `$` isCollection)
- [x] **B19** — Follow yourself (negative, no mutation): `POST /api/v1/users/:self/follow` (`:self` from `GET /me`) → **400** "cannot follow yourself" (`ErrFollowSelf`). (negative) — `backend/qa/follow_self_400.hurl` (added 2026-07-09; reuses the B16/B17 self-id capture, asserts the 400 body `error == "cannot follow yourself"` — caller==target is rejected before any follow row is created, so no cleanup needed)
- [x] **B20** — Non-admin tee create (negative): capture `course_id` from `GET /courses`; `POST /api/v1/courses/:courseId/tees` as the QA `user` → **403**. (negative — admin-only route; mirrors B2) — `backend/qa/tee_create_forbidden.hurl` (added 2026-07-14; also asserts the tee really wasn't written — a 403 that still created the row would be worse than no guard at all)
- [x] **B21** — Idempotency lookup / create-side recovery (self-cleaning): create an event under a fixed `Idempotency-Key` → `GET /api/v1/idempotency/:key` replays it (same `id` + `Idempotent-Replay: true`); an unknown key → **404**; delete the event; trailing `GET → 404`. Covers `handlers.LookupIdempotentResponse` / `DurableIdempotencyStore.Lookup` (the 7/7 phantom-create UX fix) end-to-end on live develop. — `backend/qa/idempotency_lookup.hurl` (added 2026-07-08; reuses the B12 refresh_token-as-key pattern so the key pre-exists, is byte-stable, and is unique per run)
- [x] **B21** — Non-admin external import (positive): `POST /api/v1/courses/import-external` as the QA `user` must NOT be **403**. (positive — route opened to all authenticated users on 2026-07-14; downloading a new course is no longer admin-only, only editing a downloaded course is) — `backend/qa/import_external_nonadmin_allowed.hurl` (repurposed from the old negative test; a bogus `external_id` proves the role guard is gone without polluting the shared catalog; B2/B20 stay negative since course/tee create remain admin-only)
- [x] **B22** — Scorecard settings round-trip (self-cleaning): `GET /users/me/scorecard-settings` capture the FULL settings; `PATCH` with `ob_enabled` flipped off → 200; `GET` asserts the flip + other fields preserved; `PATCH` restoring every original field → 200; final `GET` confirms exactly as found. Covers `UpsertScorecardSettings`. — `backend/qa/scorecard_settings_roundtrip.hurl` (added 2026-07-15; the PATCH is a FULL-OBJECT replace — omitted booleans persist as false — so the test captures every scalar and each of the 8 `stat_order` elements and echoes them back, flipping only `ob_enabled`; Hurl renders captured booleans as literal `true`/`false` and re-injects `stat_order` element-by-element since it can't template a whole collection)
- [x] **B23** — Event-linked round (self-cleaning): `POST /events` → 201 capture `eventId`; capture `course_id`/`tee_id` from `GET /courses`; `POST /events/:eventId/rounds` → 201 capture `roundId`; `GET /events/:eventId/rounds` shows it; `DELETE /rounds/:roundId` → 204; `DELETE /events/:eventId` → 204; trailing `GET /events/:eventId` → 404. Covers `ScheduleEventRound`. — `backend/qa/event_round_crud.hurl` (added 2026-07-20; the round-in-list assertion uses `jsonpath "$[?(@.id=='{{round_id}}')].id" == "{{round_id}}"` WITHOUT `nth 0` — a unique-id filter matches exactly one element, which Hurl unwraps to a scalar, so `nth 0` errors with "expected: list". Tears the round down before the event; frontend backlog still has no unblocked item — F11–F13 gated on the detail-screen fixture, F14–F22 done — so this run added only the backend test, with all 24 web specs green as regression)
...
- [ ] **F11** — `rounds/[id]` (round detail — **#1 most-visited screen, 99 visits/30d**): navigate to the seeded fixture round and assert the round-detail chrome renders (round name/course header + a leaderboard or scores anchor). (auth — needs the detail-screen fixture) — _deferred 2026-06-28: this run added only the backend test (B13); F11 requires first standing up the persistent detail-screen fixture (a non-self-cleaning seed), which is a deliberate task to implement on its own rather than rush at session end. The existing 10 frontend specs ran green as regression coverage._ _Still blocked 2026-07-15: F14–F22 are all done, so with F11–F13 gated on the detail-screen fixture the frontend backlog has no unblocked item — this run added only the backend test (B22). All 24 web specs ran green as regression coverage._
...
- [x] **F16** — `users/[userId]` (16 visits/30d): navigate to the QA user's own public profile (id from the session/`GET /me`) and assert the profile chrome renders (name/handle + a stats or rounds anchor). (auth — no fixture; self profile always exists) — `mobile/e2e/web/user-profile.auth.spec.ts` (added 2026-07-05; reuses the F15 navigate-by-id pattern — resolves self id from `GET /me`, reads `display_name` from `GET /users/:id`, navigates to `/users/:id`, asserts the name + the data-independent "rounds" summary line + no sign-in redirect)
- [x] **F17** — `/(tabs)/events`: tapping "Create" opens the event-creation form (name field visible), then dismissing it returns to the Events list without creating anything. (auth — data-independent, no mutation) — `mobile/e2e/web/events-create.auth.spec.ts` (added 2026-07-08; mirrors F14's non-mutating render pattern — clicks the header "Create", asserts the "Create Event" heading + the default league name placeholder, and no sign-in redirect. The modal close is an unlabelled Ionicons icon, so like F14 the spec leaves without an explicit dismiss — it never submits, so nothing is created)
- [x] **F18** — `/(tabs)/profile`: "Sign Out" returns to the sign-in screen (assert the sign-in heading/email field reappears). (auth) — `mobile/e2e/web/profile-signout.auth.spec.ts` (added 2026-07-08; on web handleSignOut confirms via `window.confirm`, so the spec auto-accepts the dialog with `page.once("dialog", …accept())`, then asserts the shared "Continue with Google" sign-in anchor reappears. Sign-out only affects the test's isolated context, not the stored session)
- [x] **F19** — `/(tabs)/profile`: toggling the theme control switches the rendered mode (assert a light/dark anchor or testID state flips), then toggle back. (auth — data-independent, no mutation) — `mobile/e2e/web/profile-theme.auth.spec.ts` (added 2026-07-08; the theme buttons have no testID on the deployed build, so the spec asserts the **selection flip** via border width — the selected option renders NativeWind `border-2`, thicker than the others — tapping Dark then Light moves the thicker border between them; restores the entry theme. Attempts to read a themed surface background color were unreliable — the largest painted element is theme-invariant — so selection-border is the stable signal)
- [x] **F20** — `/(tabs)/stats`: switching the period pill (e.g. "All Time" → another period) keeps the stats screen rendered (assert the "My Stats" heading still present). (auth — data-independent, no mutation) — `mobile/e2e/web/stats-period.auth.spec.ts` (added 2026-07-09; taps "Last 20" then restores "All Time" — both pills are always present (first/last filter options), asserts "My Stats" persists across the switch and no sign-in redirect; period switching filters in memory so nothing refetches or unmounts)
- [x] **F21** — `/(tabs)/rounds`: opening the Filter control surfaces its options and the rounds screen still renders after dismissing/applying (assert "My Rounds" persists). (auth — data-independent, no mutation) — `mobile/e2e/web/rounds-filter.auth.spec.ts` (added 2026-07-14; F11–F13 remain **blocked** on the persistent detail-screen fixture, so this run took the first unblocked frontend item. Asserts the static status options render in the sheet and the screen survives dismissal — the tab where the round list, its filters, and the persisted list-prefs store all meet, and that store now routes through utils/persistStorage.ts)
- [x] **F22** — Tab navigation persists the session: from an authenticated tab, navigate to a second tab and back, asserting both headings render (no redirect to sign-in). (auth — data-independent) — `mobile/e2e/web/tab-nav-session.auth.spec.ts` (added 2026-07-14; F11–F13 remain **blocked** on the persistent detail-screen fixture, so this run took F22, the first unblocked frontend item. Rounds→Stats→Rounds via route navigation, asserts both static headings survive and the shared "Continue with Google" sign-in anchor never appears — a session that dropped on client-side nav would strand the user on sign-in)
