...
- [x] **B21** — Non-admin external import (negative): `POST /api/v1/courses/import-external` as the QA `user` → **403**. (negative — admin-only route) — `backend/qa/import_external_forbidden.hurl` (added 2026-07-14; RequireRole runs as middleware before the handler, so the 403 holds regardless of whether GOLF_COURSE_API_KEY is set on develop — a pure authorization signal; mirrors B2/B20, no cleanup needed)
