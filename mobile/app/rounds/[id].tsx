...
export default function RoundDetailScreen() {
  ...
  const openEditGroup = (group: RoundGroup) => {
    // Groups from the API send tee_time as "h:mm AM/PM"; the picker needs "HH:MM".
    setEditGroupId(group.id);
    setEditGroupName(group.name ?? "");
    setEditGroupTeeTime(parseFormattedTeeTime(group.tee_time));
  };
          ...
          <ScrollView>
            <View className="px-5 pt-8 pb-10">
              ...
              <View className="mb-6">
                ...
                {/* TimeInput handles the native picker (iOS/Android) and the
                    browser-native <input type="time"> on web. */}
                <TimeInput
                  value={editGroupTeeTime}
                  onChange={setEditGroupTeeTime}
                  disabled={updateGroupMutation.isPending}
                  clearable
                />
              </View>
